...
start = time.time()
case, sup, inpath, outpath = sys.argv[-4], int(sys.argv[-3]), sys.argv[-2], sys.argv[-1]

def load(x):
    ids = x.split(',')
    return (str(ids[0]), str(ids[1])) if case == '1' else (str(ids[1]), str(ids[0]))
...
lines = sc.textFile(inpath).filter(lambda x: x != ('user_id,business_id'))
bask = lines.map(load).distinct()\
            .groupByKey() \
